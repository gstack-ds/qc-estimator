...
- **2026-06-25 — Gmail scanner outage recovered + durability fixed (branch `fix/scanner-durability`):**
  ...
  - **Still TODO:** no heartbeat/alerting — the 13-day outage was silent. `notify.ts` exists; add a "no successful scan in N hours" notification so the next failure isn't invisible.
