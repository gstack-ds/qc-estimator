...
- **MIGRATION SEQUENCE:** repo files run = 050,051,052,054,055. **053 is an intentional gap** — a migration Gary ran manually in Supabase that was never committed as a repo file (likely the market-data cleanup). Next new migration = **056**. If Gary says what 053 was, add it as `053_*.sql` so a fresh DB rebuild matches prod.
