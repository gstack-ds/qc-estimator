...
### Next Session Start
- **2026-06-25 — Gmail scanner outage recovered + durability fixed (branch `fix/scanner-durability`):**
  - **Root cause of missed leads:** scanner was DEAD since June 12 16:00 (~13 days). PM2 daemon was killed (no reboot — uptime since June 10) and `pm2 startup` is unconfigured on Windows, so it never came back. Error log empty = no crash, just vanished. NOT a parse/filter/dedup bug.
  - **Recovery:** ran `npm run backfill -- --after=2026-06-11` → recovered 2 leads (Atlanta = Classic Conferences/MIMEDX, assigned Khloe; Savannah = PNC Bank, assigned Lindsey), 1 already in DB, 0 dupes. The Atlanta lead Alex flagged was just an outage victim — parsed cleanly, matched `subject:"INITIAL LEAD"`.
  - **Durability fix:** replaced PM2 + node-cron with a Windows Task Scheduler one-shot (`scripts/run-scan-once.ts` → `build:scan-once` → `run-scan-once.js`, launched by `run-scan-once.bat`, 7-day lookback). PM2 scanner removed. A non-elevated task "QC Lead Scanner" IS registered now (current-user, runs at logon + 4x daily ET) and tested green (LastTaskResult 0, next run 2pm). This already auto-relaunches at logon — strictly better than the old PM2 setup.
  - **⚠ ACTION PENDING (needs Gary, elevated):** the box has auto-login OFF, so the current logon-only task does NOT survive a reboot-without-login. Run this ONCE in an **elevated** PowerShell to upgrade to boot-durable (S4U, "whether logged on or not"):
    `powershell -ExecutionPolicy Bypass -File "scripts\install-scanner-task.ps1"`
    Then verify: `Get-ScheduledTaskInfo -TaskName "QC Lead Scanner"` (LastTaskResult should be 0).
  - **Heartbeat/alerting DONE (branch `feat/scanner-heartbeat`, merged):** scanner writes `data/last-scan.json` on each successful scan; separate "QC Lead Scanner Watchdog" task (12:00+17:00 ET) emails an alert via notify.ts if no successful scan in 18h. Fixed a latent bug: notify.ts read `NOTIFY_EMAIL` but prod .env has `NOTIFICATION_RECIPIENTS` → notifications had NEVER sent; now resolves both. `run-watchdog --test` sends a labelled test alert. **⚠ During verification a real (false) alert reached Alex+Lindsey — they were told it was a test.**
    - **⚠ ACTION PENDING (needs Gary, elevated):** re-run `powershell -ExecutionPolicy Bypass -File "scripts\install-scanner-task.ps1"` (elevated) to register the new **Watchdog** task (it also re-registers the scanner — idempotent). Until then the watchdog isn't scheduled.
    - Known gap: alert channel is Gmail, so a Gmail-auth failure can't self-report. External watcher (Vercel cron + DB heartbeat) deferred.
- **961 tests passing.** On main, deployed to production (latest merge `1b3c0db`).
