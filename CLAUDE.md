...
### Next Session Start
- 902 tests passing. On main, deployed to production (commit 9f384f5).
- **DONE this session (2026-06-15):** (1) Deck-title leak fix — `sanitizeEstimateName` switched from the broken blanket regex (it destroyed "WORLD OF COCA-COLA") to a DENYLIST keyed on team initials + upcharge; audited live (22 junk strips, 0 false positives); merged + deployed. (2) Estimate **assigned_to + internal_notes** feature (migration 047, ALREADY RUN in prod) — assignee initials badge + dropdown on estimate cards (5 tool users; Alex shows "AQS"), "Internal — not shown on proposals" textarea on all 5 builders; leak-proof by exclusion from RawEstimate/DeckContract/ProposalDocument; merged + deployed.
