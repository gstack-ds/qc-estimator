...
| 2026-06-22 | Budget feature data model + leak/write security | Two stored toggles (aggregation sum\|select_one × tiered) presented as ONE 3-way Mode. Budget derives from real estimates → CLIENT total + pp ONLY (deriveEstimates never returns cost/markup/margin → leak-safe by construction). Public surfaces (`/b/[token]` view + `/api/budget/[token]/respond` write) NEVER get a browser Supabase client: read/write via service-role, scoped to a token-hash lookup; NO anon RLS policy. Snapshot is the leak boundary (BudgetShareContract strips sourceEstimateId + line notes; leak test gates it). Client capture recomputes the total server-side from the LOCKED snapshot — never trusts a client number; payload is Zod-shaped + snapshot-validated (only references to existing options survive). | Manual tiers (rejected — derive from estimates); anon RLS write policy (rejected — service-role-only); trusting client total (rejected — recompute) |
| 2026-06-22 | Proposal PDF + exports group line items by section ID, not display name | Two sections can share a name (e.g. two "FLORALS"); name-based grouping double-emitted shared-name items. `groupLineItemsBySections` (export.ts) keys off sectionId (falls back to name only when no id). orderedSections passed as `{id,name}[]`. | Group by name (rejected — same-name collision); dedupe names (rejected — loses distinct sections) |
| 2026-06-25 | Scanner heartbeat + independent watchdog task for silent-failure alerting | The 13-day outage was invisible because nothing watched the watcher. Scanner writes `data/last-scan.json` (file-based, mirrors dedup.ts) on every successful Gmail scan (`recordHeartbeat` in heartbeat.ts, called in index.ts right after `scanGmail` succeeds — 0 messages still counts as alive). A SEPARATE "QC Lead Scanner Watchdog" task (12:00 + 17:00 ET) runs `run-watchdog.ts`, which alerts via notify.ts if no successful scan in 18h (clears the 15h overnight gap; null heartbeat also = stale). Boot-grace guard (skip if uptime <30m) avoids a false alarm after an intentional power-off. `run-watchdog --test` sends a labelled test alert to verify delivery. Watchdog is a separate task so it fires even when the scanner is dead. Known gap: alert channel is Gmail (notify.ts), so a Gmail-auth failure can't self-report — an external Vercel-cron + DB-heartbeat watcher would close that, deferred. | Heartbeat in DB (rejected — adds DB dep + migration; file matches dedup); watchdog inside the scanner (rejected — can't detect its own death); Vercel cron external watcher (deferred — more infra) |
| 2026-06-25 | Gmail scanner moved from PM2 + node-cron daemon to a Windows Task Scheduler one-shot | PM2 had no boot resurrect on this box (`pm2 startup` → "Init system not found") and the daemon silently died during unrelated work, losing 13 days of leads. Task Scheduler is OS-native, boot-level, and — crucially — there is NO long-lived process to silently die between scans. `scripts/run-scan-once.ts` (compiled via `build:scan-once`) calls `runScan()` once with a 7-day lookback (dedup makes overlap free, so a missed run self-heals) and exits. Launched by `scripts/run-scan-once.bat` (sets cwd so dedup's `process.cwd()` finds `data/processed-ids.json`). Triggers: 7/11/14/16 ET + at boot; box is Eastern so trigger-local time == ET. Install via `scripts/install-scanner-task.ps1` (elevated). PM2 scanner removed (`pm2 delete` + `pm2 save --force`) — never run both (double-scan). | pm2-windows-startup (rejected — hooks logon not boot; dead if no one logs in); pm2-installer (heavier, keeps PM2 the failed component); keep daemon under Task Scheduler (awkward — Scheduler isn't a daemon supervisor) |
| 2026-06-29 | Excel export money columns formatted as currency via SheetJS cell `.z`, not string concatenation | Alex pasted exports into Canva and had to reformat raw numbers. `applyCurrencyFormat` (ExportButtons.tsx) sets `cell.z = '$#,##0.00'` on numeric cells only — values stay real numbers (sortable, no re-parse). Client Summary col B; Detail cols Unit Price/Our Cost/Client Cost. | String concat "$1,234" (rejected — breaks numeric cells); reformat every export by hand (the status quo) |
| 2026-06-29 | Leads date entry: one shared `FlexibleDateInput` + pure parser, replacing native `<input type="date">` | Native date input forces segmented MM/DD/YYYY, a 4-digit year, and blocks paste — painful for rapid entry of future dates from emails. `dateParse.ts` (server-free, 41 tests) parses M/D/YY, M/D/YYYY, ISO, "May 11 2027", "11 May", bare M/D → canonical YYYY-MM-DD. Bare M/D → current year; 2-digit → 20YY; ambiguous → US M/D. Value built from numeric PARTS as a string — never `new Date('YYYY-MM-DD')` (UTC-parse shifts a day in ET). Unparseable text kept with a red ring (never wiped); optional 📅 picker retained. Used in card + table cell + Add-Lead panel. | Keep native input (the complaint); per-call-site bespoke inputs (rejected — 3 copies); a date-picker library (heavier; doesn't fix the typing-first need) |
| 2026-06-29 | Proposal PDF sections are wrappable (not `wrap={false}`); header pair kept together via `wrap={false} minPresenceAhead={60}`, each row `wrap={false}` | A `wrap={false}` section taller than a page made @react-pdf emit "Node of type VIEW can't wrap … bigger than available page height" and collapse the block's layout (header-on-row, tax-rate-on-jurisdiction, compressed rows) on the overflow page — only on long floral/decor sections, page 2. Making the section wrappable lets it flow; the header pair + individual rows stay intact. | `wrap={false}` on the whole section (the bug); break-before-each-section (doesn't fix an oversized single section) |
| 2026-06-29 | Tax column shows the STATE abbreviation ("6% VA"), not the city; one shared `stateAbbrevFromLocation` helper | `shortLocationName` stripped the trailing state abbrev but left the comma → "Middleburg," for "Middleburg, VA". Alex wants the state. `src/lib/utils/locationLabel.ts` (server-free, 6 tests) returns the state for "City, ST" / "City (ST)" / "" when none; both render sites (ProposalDocument PDF + LineItemRow on-screen) use it and guard the empty case (no dangling comma, no blank line). Duplicate local helpers removed so they can't drift. | Keep the city + strip the comma (rejected — Alex wants state); fix one site only (drift) |
| 2026-06-29 | EEG commission base = Pre-Tax Total (Subtotal + Production Fee), not the pre-fee subtotal | Alex confirmed the 10% applies to the Pre-Tax Total line (the line right before Tax), incl. the production fee. Engine: `eegCommissionAmount = rate × preTaxTotal`. (Initial build used `lineItemsSubtotalClient`; corrected before merge.) | rate × subtotal (pre-fee — wrong per Alex); rate × grand total (would tax-on-tax) |
| 2026-06-29 | EEG commission is a margin-neutral pass-through (added to client total, backed out of margin) | "Commission paid to a third party (EEG)" → QC collects it for EEG, keeps nothing. `calculateMarginAnalysis` uses `marginClientTotal = totalClient − eegCommissionAmount` everywhere it used totalClient (GDP base, qcRevenue, margin %, team-hours, true-net), so QC margin $ and % are identical on/off. Per-estimate toggle + editable rate (default 10%); migration 056 (eeg_enabled, eeg_rate). Client-visible line after tax; itself untaxed; does not change the tax figure. | Mirror the discount literally (would add EEG to QC revenue — wrong for a pass-through); leave margin untouched (silently inflates margin by the full commission) |
| 2026-06-29 | Program-level combined proposal: each estimate its own page-broken section; grand total sums per-estimate computed totals; per-estimate tax kept separate | Reuses the per-estimate ProposalDocument blocks (extracted EstimateLineItemSections / EstimateTotals / TourLogisticsBlock — carry the wrap fix + section-id grouping). `computeProgramGrandTotal` sums lineItemsSubtotalClient/productionFee/preTaxTotal/own-tax/discount/eeg/total. Per-estimate summary via the shared `buildDeckContract` (engine untouched). Client-rendered via dynamic import (server action gathers payloads). v1: program travel excluded from the combined doc (folding it per-estimate would (N−1)×over-bill); header page-1-only; transportation excluded (ProposalDocument supports venue/av/decor/tour). No migration. | Blend all estimates' tax into one figure (rejected — different jurisdictions); a new engine path (rejected — sum existing totals); include travel per-estimate (over-bills N×) |
...
| 2026-06-22 | UPDATE via the session client on a table with only a SELECT RLS policy = silent 0-row no-op (no error) | budget_share_responses has only an authenticated SELECT policy (writes are service-role). markResponsesViewed first used `createClient()` (session) `.update()` → 0 rows changed, no error, badge never cleared. **Rule going forward:** any table whose design is "writes via service-role only" must do its server-action writes with the service-role admin client (auth-gated + code-scoped), NOT the session client — the session UPDATE fails silently against a SELECT-only policy. Recurring class: applies to budget_shares, budget_share_responses, and any future service-role-write table. |
| 2026-06-22 | Migration number 053 was used by a manually-run (uncommitted) Supabase migration | The repo's `053_budget_share_responses.sql` collided with a 053 Gary had already applied by hand. Renamed to 054. **Rule:** repo migration files are not the only migrations that ran in prod — Gary applies some ad-hoc SQL directly. Before numbering a new migration, confirm the next free number with Gary if there's any doubt; current next free = 057 (056 used by EEG commission). |
| 2026-06-25 | Gmail scanner was dead for 13 days with no alert — PM2 daemon terminated and never came back | Root cause: PM2 daemon was killed (likely during the June 14 deck-renderer PM2 work — the gotcha there says PM2 must be stopped to delete the locked dir) and `pm2 startup` returns "Init system not found" on Windows, so there was no boot/crash resurrect. Error log was empty (no crash exception) — it just vanished. **Detection gap:** nobody noticed for 13 days because there's no heartbeat/"no successful scan in N hours" alert. Fixed durability by moving to Task Scheduler (see Design Decisions 2026-06-25); alerting still TODO. |
| 2026-06-25 | notify.ts read `NOTIFY_EMAIL` but prod .env sets `NOTIFICATION_RECIPIENTS` — scanner notifications silently never sent | Naming mismatch between the code (`NOTIFY_EMAIL`, from .env.example) and the deployed .env (`NOTIFICATION_RECIPIENTS`, from PRD-leads.md). `getRecipients()` now resolves `NOTIFICATION_RECIPIENTS ?? NOTIFY_EMAIL` (comma-separated list). Lesson: an env-keyed feature can be a no-op forever if no one verifies a message actually sent — exactly the silence the heartbeat now guards against. |
| 2026-06-25 | PowerShell `$env:VAR = ""` does NOT reliably pass an empty var to a Node child — Windows drops empty-string vars from the child env block, so dotenv reloads the real .env value | While testing the watchdog I set `$env:NOTIFICATION_RECIPIENTS = ""` to force the no-send path; the child node process never received it, dotenv loaded the real recipients, and a real (false) alert emailed the team. To truly suppress for a child: `Remove-Item Env:\VAR` AND ensure dotenv won't reload it, or point the script at a throwaway env. **Always confirm an outward-facing test won't actually send before running it.** |
| 2026-06-29 | Budget showed $125/pp inside an estimate but $250/pp on the program page (same estimate) | A `budget_plan_entries` row carried the right value/event (Resident Leadership Forum, 125/pp, `linked_event_id` correct) but its `linked_estimate_id` pointed at a DIFFERENT same-named restaurant estimate (Ecco Midtown / Carousel event, real budget 250/pp event-level). `resolveEstimateBudget` precedence #1 (estimate-linked entry) then surfaced the wrong budget in the snapshot bar + that card's badge. The "exactly 2×" was coincidence of the two events' budgets, NOT a divide bug. Fixed by repointing the FK (1 row). A global mismatch scan (linked estimate's `event_id` ≠ entry's `linked_event_id`) found only this one. **Watch:** same-name estimates make budget-entry estimate-links easy to mis-attach; optional hardening = have the resolver ignore an estimate-linked entry whose `linked_event_id` ≠ the estimate's event (not yet done). |
| 2026-06-25 | Registering a Task Scheduler task that "runs whether logged on or not" (S4U) or as SYSTEM requires an ELEVATED shell | A non-elevated `Register-ScheduledTask -LogonType S4U` fails with "Access is denied." A logon-only (`-LogonType Interactive`) current-user task registers fine without admin but only runs once the user logs in — and this box has auto-login OFF, so a logon-only task does NOT survive a reboot-without-login. The boot-durable install (`scripts/install-scanner-task.ps1`) must be run from an elevated PowerShell. |
| 2026-06-29 | DB schema mismatches (e.g. `programs.client_company` — should be `company_name`) only surface at PREVIEW, not in tests | The combined-proposal gather action selected a non-existent `client_company` column → wizard crashed at runtime on step 2. Unit tests never hit the DB, so `npx tsc` + the suite were both green. The PREVIEW run is the integration check for any server action that queries Supabase. When writing a new `.select()`, verify each column against `supabase/migrations/*.sql` (source of truth) or copy from a known-working select (queries.ts / fetchSlide). Real client/company columns on programs: `client_name`, `company_name`. |
| 2026-06-29 | @react-pdf `wrap={false}` on a node taller than a page collapses its layout (not just clips) | The proposal-PDF overlap bug. The engine WARNS ("Node of type VIEW can't wrap between pages and it's bigger than available page height") and crams all children to the top. Never put `wrap={false}` on an unbounded-height container (a whole section). Keep it only on bounded pieces (a header pair, a single row). Verified via an isolated `ReactPDF.renderToFile` repro. |
...
### Next Session Start
- **2026-06-29 SESSION B — proposal PDF + EEG + program export, all shipped to production (1047 tests passing; latest merge `74180c0`):**
  1. **Proposal PDF section-wrap fix** (merge `8616ebf`). Long floral/decor sections overlapped on page 2 — root cause: `wrap={false}` on a section taller than a page (@react-pdf collapses it). Sections now wrappable; header pair + rows stay intact. Regression guard `tests/unit/proposal-pdf-wrap.test.ts` (renders a 45-item section, asserts no overflow warning; fails on pre-fix code). Rollback was `a4e7203`.
  2. **Tax column shows state not city** (merge `56fc627`). "Middleburg," (dangling comma) → "6% VA". Shared `src/lib/utils/locationLabel.ts::stateAbbrevFromLocation` used by both ProposalDocument (PDF) and LineItemRow (on-screen); duplicate local helpers removed. Display-only — tax rate untouched. Rollback was `8616ebf`.
  3. **EEG commission** (merge `7824481`, **migration 056** run in prod). Per-estimate, client-visible, after-tax, **margin-neutral pass-through**, base = **Pre-Tax Total (Subtotal + Production Fee)**. Toggle + editable rate (default 10%) in all 4 builders; row after tax in 3 summary panels + PDF; EEG added to BOTH Copy Numbers export paths (reconciliation). Rollback was `56fc627`.
  4. **Program-level combined proposal export** (merge `74180c0`, no migration). "Export Full Proposal" wizard on the program page (select/deselect → drag-reorder → generate). Each estimate = its own page-broken section reusing the per-estimate blocks; per-estimate tax kept separate; program grand total sums computed totals. `ProposalDocument` refactored into exported sub-components (EstimateLineItemSections / EstimateTotals / TourLogisticsBlock); new `ProgramProposalDocument` + `src/lib/proposals/programProposal.ts` (pure, 15 tests) + `getProgramProposalData` server action + `ExportFullProposalButton`. v1 defaults: program travel excluded from combined doc, header page-1-only, transportation excluded. Rollback = **`7824481`**.
  - **Branches deleted (all merged):** fix/proposal-pdf-section-wrap, fix/proposal-tax-state-label, feat/eeg-commission, feat/program-proposal-export.
  - **Open follow-ups (shelved, Gary's call):** (a) Client Discount missing from Copy Numbers/Excel export rows — pre-existing reconciliation gap (a discounted estimate's rows don't sum to its total); (b) combined doc: bill program travel ONCE; (c) repeat the program header on every page of the combined doc; (d) transportation support in the combined export; (e) backfill the 053 migration file (manual gap — next free = 057).
- **2026-06-29 SESSION — all shipped to production (1010 tests passing; latest merge `9408525`):**
  1. **Scanner outage recovery + durability (DONE + verified).** Scanner was DEAD ~13 days (PM2 daemon killed during the June 14 deck-renderer work; `pm2 startup` unconfigured on Windows → no resurrect; empty error log = it just vanished, not a parse bug). Recovered 2 missed leads via `npm run backfill -- --after=2026-06-11` (incl. the Atlanta lead Alex flagged — an outage victim). Replaced PM2+node-cron with a **Windows Task Scheduler S4U one-shot** (`run-scan-once.ts`→`build:scan-once`, boot + 4x daily ET, 7-day lookback). Gary ran `install-scanner-task.ps1` elevated and confirmed S4U registered + LastTaskResult 0. PM2 scanner removed (`pm2 delete` + `pm2 save --force`).
  2. **Scanner heartbeat + watchdog (`feat/scanner-heartbeat`, merged).** `data/last-scan.json` written each successful scan; a SEPARATE "QC Lead Scanner Watchdog" task (12:00+17:00 ET) emails via notify.ts if no successful scan in 18h. Fixed a latent bug — notify.ts read `NOTIFY_EMAIL` but prod .env has `NOTIFICATION_RECIPIENTS`, so scanner notifications had NEVER sent; `getRecipients()` now resolves both. `run-watchdog --test` = labelled delivery check.
     - **⚠ STILL PENDING (needs Gary, elevated):** the watchdog TASK is not scheduled until you re-run `powershell -ExecutionPolicy Bypass -File "scripts\install-scanner-task.ps1"` (elevated) — it registers the watchdog (and idempotently re-registers the scanner). The code is merged; only the task registration remains.
     - Known gap (deferred): alert channel is Gmail, so a Gmail-auth failure can't self-report. External Vercel-cron + DB-heartbeat watcher would close it.
  3. **Excel export currency (`fix/xlsx-currency-format`, merged+deployed).** Money columns now `$#,##0.00` via SheetJS cell `.z` (stay real numbers) so Alex doesn't reformat after pasting into Canva.
  4. **Budget per-person $125-vs-$250 bug (data fix, 1 row).** A mis-linked `budget_plan_entry` (`34b9e43b`, Resident Leadership Forum $125/pp) had `linked_estimate_id` pointing at the Carousel/Ecco Midtown estimate (`d711a3cb`, whose real budget is event-level 250/pp). Repointed to `c9289f73` (the Resident Leadership estimate). The "exactly 2×" was coincidence — engine + resolver untouched. Global scan found ONLY this one mislink.
  5. **Flexible date entry for leads (`feat/flexible-date-input`, merged+deployed).** Replaced native `<input type="date">` (segmented MM/DD/YYYY, no paste — Alex's "nightmare typing") with one shared `FlexibleDateInput` across Kanban card / table cell / Add-Lead panel. Free typing + paste + optional 📅 picker; parser `src/lib/leads/dateParse.ts` (41 tests).
- **Branches safe to delete (all merged this session):** fix/scanner-durability, feat/scanner-heartbeat, fix/xlsx-currency-format, feat/flexible-date-input.
- **3 untracked test-fixture files** in `tests/fixtures/` (2 PDFs + 1 HTML) — present since session start, not committed. Decide: track them or add to `.gitignore`.
- **DONE this session (2026-06-22) — the BUDGET feature, end to end + several live-bug fixes:**
  - **Phase 1** (migration 051): internal budget builder at `/programs/[id]/budget`. `budget_documents`/`budget_lines`/`budget_line_members`. Pure engine `src/lib/budget/budgetDocument.ts`. Two stored toggles (aggregation sum|select_one × tiered) exposed as ONE 3-way Mode (Add up / Pick one / Low-Mid-High). Derives from real estimates (`deriveEstimates.ts` → client total + pp ONLY) with per-tier override; combine/break-out; refresh-from-estimates; client-safe Preview.
  - **Phase 2** (migration 052): public view-only share link `/b/[token]`. Server-rendered from a stored client-safe snapshot via service-role; middleware allow-lists `/b/`; 256-bit token, hash-only storage; expiry + revoke/regenerate. Leak-proof `budgetShareContract.ts` (strips sourceEstimateId + line notes) + leak test (`budgetShare.test.ts`).
  - **Link management:** one active link per budget (createBudgetShare insert-first then revoke-others, failure-safe) + central `/share-links` dashboard (revoke / revoke-all).
  - **Phase 3** (migration 054 — renumbered from 053, see below): public client capture. `POST /api/budget/[token]/respond` → token-validate (revoked/expired→410, nothing written) → Zod → validate against locked snapshot → recompute total server-side → write `budget_share_responses` only. `budgetResponse.ts` validator + gate tests. Alex's Responses panel.
  - **Phase 4 Part 1** (migration 055): in-app unread indicator (shared team pool). `viewed_at` on responses; nav badge on Programs; "Budget Responses" filter tab; deliberate per-response "Mark as read" (NOT clear-on-view); snap-to-program from the badge.
  - **Live-bug fixes:** menu import created one line per dish → now ONE line + menuSelections detail, both import paths (`menuImport.ts`/`vendorProfileMapping.ts`); proposal PDF duplicated same-named sections → group by section ID not name (`groupLineItemsBySections` in `export.ts`).
- **MIGRATION SEQUENCE:** repo files run = 050,051,052,054,055,056. **053 is an intentional gap** — a migration Gary ran manually in Supabase that was never committed as a repo file (likely the market-data cleanup). Next new migration = **057**. If Gary says what 053 was, add it as `053_*.sql` so a fresh DB rebuild matches prod.
- **BUDGET deferred:** Phase 4 **Part 2 = email notifications** (Resend/SES + DNS/SPF/DKIM on qceventdesign.com — the long pole; in-app indicator is the non-email fallback already shipped). Also: auto-apply client selections to the estimate (deliberately NOT built — manual review only); responder identity (not captured).
- **Branches safe to delete (all merged):** feat/budget-builder-phase1, feat/budget-share-link, feat/budget-link-management, feat/budget-capture-phase3, feat/response-notifications, fix/menu-import-single-line, fix/proposal-section-dedup.
- **NEXT (carried over): one-time cleanup of the ~15 legacy estimate names** with notes baked into the title (e.g. "Fado Irish Pub - DR (upcharge at 40%)" → name "Fado Irish Pub", assigned Danielle, internal_notes "upcharge at 40%"). Use `scripts/check-sanitize-names.ts` to list affected rows. DB raw names unchanged; only deck/proposal OUTPUT is sanitized.
