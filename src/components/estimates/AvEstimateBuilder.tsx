...
export default function AvEstimateBuilder({
  ...
  const handlePopulateFromExtraction = useCallback((data: ExtractedData) => {
    ...
    const toImport: LocalLineItem[] = (data.equipmentItems ?? []).map((item) => {
      ...
      const sectionDef = isLabor ? staffingSection : equipSection;
          ...
          <div className="bg-white border border-brand-cream rounded-lg p-5 space-y-6">
            {selectedItems.size > 0 && (
              <div className="flex items-center gap-3 bg-brand-offwhite border border-brand-copper/30 rounded px-3 py-2 text-sm flex-wrap">
                ...
                <select className="border border-brand-cream rounded px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-brand-copper bg-white text-brand-charcoal" defaultValue="" onChange={(e) => { if (e.target.value) handleMoveToSection(e.target.value); }}>
