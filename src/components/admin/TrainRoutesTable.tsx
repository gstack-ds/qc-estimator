...
import { upsertTrainRoute, deleteTrainRoute } from '@/app/(admin)/admin/actions';
import type { DbTrainRoute } from '@/lib/supabase/queries';

type Row = DbTrainRoute & { isNew?: boolean };
...
export default function TrainRoutesTable({ initialData }: { initialData: DbTrainRoute[] }) {
  ...
  const saveRow = useCallback(async (row: Row) => {
    setSaveError(null);
    const result = await upsertTrainRoute({ id: row.isNew ? undefined : row.id, route_name: row.route_name, low_cost: row.low_cost, high_cost: row.high_cost });
  ...
  const addRow = () => { const id = `new-${Date.now()}`; setRows((p) => [...p, { id, route_name: 'New Route', low_cost: 0, high_cost: 0, updated_at: '', isNew: true }]); setEditingCell({ rowId: id, col: 'route_name' }); };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="text-sm font-semibold text-brand-charcoal">Train Routes</h3>
          ...
          <tbody className="divide-y divide-brand-cream/60">
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-brand-offwhite transition-colors">
                {(['route_name', 'low_cost', 'high_cost'] as const).map((col) => {
                  const isEditing = editingCell?.rowId === row.id && editingCell?.col === col;
                  const isNum = col !== 'route_name';
                  const display = isNum ? formatCurrency(row[col] as number) : row.route_name;
                  return (
