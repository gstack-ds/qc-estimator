// Shared read-only retrieval handlers (programs). Live here so BOTH the MCP server and the
// in-app chatbot import one source. Each handler takes an injected SupabaseClient and plain args.
import type { SupabaseClient } from '@supabase/supabase-js';
import type { ListProgramsArgs, GetByIdArgs } from './types';
